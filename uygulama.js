...
function panelArz() {
  ...
  const ozetKart = `<div class="kart">
    <div class="baslikSatir">
      <h2>${arzYilSecim} halka arz karnesi</h2>
      <div class="dugmeSira" style="margin-left:auto">
        ${['2026', '2025', '2024'].map(y => `<button class="dugme" data-yil="${y}" aria-pressed="${y === arzYilSecim}">${y}</button>`).join('')}
      </div>
    </div>
    <div class="izgara i4">
...
function arzListesi() {
  const hepsi = V.arz.arzlar.filter(a => a.yil === arzYilSecim);
  const perfli = hepsi.filter(a => a.perf?.getiri != null);
  const sirala = {
  ...
  const satirlar = [...hepsi].sort(sirala).map(a => {
    const p = a.perf;
    return `<tr>
      ...
      <td style="max-width:210px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:12.5px">${kaks(a.ad)}</td>
      ...
      <td class="say">${p ? para(p.guncelFiyat, 2) : '—'}</td>
  ...
  return `<div class="kart">
    <div class="baslikSatir">
      <h2>${arzYilSecim} arzlarının tamamı</h2>
      <span class="not">${hepsi.length} arz · ${perfli.length}’i işlem görüyor</span>
      <div class="dugmeSira" style="margin-left:auto">
        ${[['getiri', 'Getiriye göre'], ['tarih', 'Tarihe göre']].map(([k, ad]) =>
          `<button class="dugme" data-sira="${k}" aria-pressed="${k === arzSiraSecim}">${ad}</button>`).join('')}
      </div>
    </div>
    <div class="tabloKutu"><table>
      <thead><tr>
        <th>Kod</th><th>Şirket</th><th>Talep toplama</th>
        <th class="say">Arz ₺</th><th class="say">Bugün ₺</th>
    ...
    <div class="kaynakNot" style="margin-top:12px">
      ...
      “BIST’e göre” aynı dönemde BIST 100’ün getirisi düşülmüş halidir — arzın endeksin üstünde mi kaldığını gösterir.
      “Kişi başı”, bireysel yatırımcıya düşen ortalama lotun arz fiyatıyla çarpımıdır.
    </div>
  </div>`;
}
...
function sekmeAc(ad, zorla = false) {
...
// Panel içindeki düğmeler (yıl / sıralama) olay delegasyonuyla.
document.addEventListener('click', e => {
  const yil = e.target.closest('[data-yil]');
  if (yil) { arzYilSecim = yil.dataset.yil; sekmeAc('arz', true); return; }
  const sira = e.target.closest('[data-sira]');
  if (sira) { arzSiraSecim = sira.dataset.sira; sekmeAc('arz', true); return; }
